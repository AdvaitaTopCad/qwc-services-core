...
class RuntimeConfig:
    ...
    def read_config(self, tenant):
        ...
        :param str tenant: Tenant ID
        """
        runtime_config_path = RuntimeConfig.config_file_path(self.service, tenant)
            ...
            try:
                if val is None:
                    ...
                    if envval.lower() in ("true", "t", "1", "on", "yes", "y"):
                        ...
                        raise Exception("Unknown boolean value for '%s'" % envval)
                else:
                    val = type(val)(envval)
            except Exception as e:
                self.logger.warning(
                    "Could not convert config override "
                    "from env '%s=%s' to %s:\n%s" % (name.upper(), envval, type(val), e)
                )
